...
def plot_baseline(plot_choice):
    if plot_choice.lower() == 'baseline':
        ...
        for sensor_data in sensors_files:
            ...
            # Customize layout
            fig.update_layout(
                title=f"TOF Stacker Comparison: {sensor_data}",
                xaxis_title="Bins",
                yaxis_title="Photon Count",
                legend_title=f'Zone: {zone}',
                template="plotly_white",
...
def plot_tests(hashes, labware, axis):
    ...
    if labware:
        for hash in hashes:
            matches = df[(df['Hash_id'] == hash) & (df['Labware Stacked'] == 1) & (df['Axis'] == axis)]
            for i, match in enumerate(matches.itertuples()):
                ...
                fig.add_trace(go.Scatter(x=bins, y=zone_data, mode='lines', name=f'{labware_name}_{i+1}'))
        fig.update_layout(
        title=f"TOF Baseline Test: {axis}",
        xaxis_title="Bins",
        yaxis_title="Photon Count",
        legend_title=f'Zone: {zone}',
        template="plotly_white",
...
def test_baseline(baseline, df_path):
    ...
    plot_tests(hashes_no_lab, False, axis)
    # For labware
    ...
    plot_tests(hashes_lab, True, axis)
...
def menu():
    ...
    if options[selection_int] == 'Make Baseline':
        ...
        file_location = input('Enter csv file: (type all to validate entre df)')
        print('x')
        print('z')
