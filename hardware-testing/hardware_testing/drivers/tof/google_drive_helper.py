...
def upload_files(drive, local_storage, email):
    drive = google_drive(drive.credentials, drive.create_folder("Files"), email)
    try:
        ...
        traceback.print_exc()
    os.rmdir(local_storage)
def upload_plots(drive, local_storage, email):
    drive = google_drive(drive.credentials, drive.create_folder("Plots"), email)
    try:
        for file in os.listdir(local_storage):
            drive.upload_file(file)
    except:
...
def upload_tof_files(drive_details, axis, labware_num):
    ...
    tof_logs = os.path.join(curr_dir, "tof_logs")
    tof_plots = os.path.join(curr_dir, "tof_plots")
    if axis == 'x':
        drive_details = drive_details[0:3] + ['X-Axis'] + drive_details[5:-1] + [labware_num]
    elif axis == 'z':
        drive_details = drive_details[0:5] + ['Z-Axis'] + drive_details[7:-1] + [labware_num]

    components = drive.list_folder(folder=True)

    print(f"test {components} \nparent folder: {drive.parent_folder}\n")
    for path_comp in drive_details[2: ]:
        if path_comp not in components:
            ...
            components = drive.list_folder()
    upload_files(drive, tof_logs)
    upload_plots(drive, tof_plots)
